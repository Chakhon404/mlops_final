...
# ==============================
def main():
    # 🔹 แก้ปัญหา path Windows บน GitHub Actions (Linux)
    ...
    with mlflow.start_run():
        ...
        majority_prop = _safe_ratio(
            int(label_counts.max() if len(label_counts) else 0),
            int(label_counts.sum() if len(label_counts) else 0)
        )
